...
class TripPayment:
    @classmethod
    def initiate_payment(self,trip:Trip,request):
        url = 'https://api.edfapay.com/payment/initiate'
        trip_number = str(trip.pk)
        trip_amount = str(trip.price_with_tax_with_coupon())
        trip_currency = "SAR"
        merchant_password = str(os.getenv('MERCHANT_PASSWORD'))
        names = trip.client.fullName.split()
        firstName = names[0]
        lastName = names[-1]
        trip_description=f'payment trip with %.2f {trip_currency}' % trip.price_with_tax_with_coupon()
        trip_id=f"TR{trip.id}"
        client_ip, _ = get_client_ip(request)
        if client_ip is None:
            # Unable to get the client's IP address
            return
        signature = generate_signature(trip_id, trip_amount, trip_currency, trip_description, merchant_password)
        payload = {
            'action': 'SALE',
            'edfa_merchant_id': str(os.getenv('MERCHANT_KEY')),
            'order_id': trip_id,
            'order_amount': trip_amount,
            'order_currency': trip_currency,
            'order_description': trip_description,
            'req_token': 'Y',
            'payer_first_name': str(firstName),
            'payer_last_name': str(lastName),
            'payer_address': str(trip.client.address),
            'payer_country': 'SA',
            'payer_city': 'Riyadh',
            'payer_zip': '00000',
            'payer_email': str(trip.client.email),
            'payer_phone': str(trip.client.phone),
            'payer_ip': str(client_ip),
            'term_url_3ds': request.build_absolute_uri(reverse('finalize_payment')),
            'recurring_init': 'N',
            'auth': 'N',
...
def payment_order_handeler(order_id,trans_id,amount):
    ...
    driver=order.driver
    payment_order=PaymentOrder.objects.create(trans_id=trans_id,order=order,amount=amount,confirmed=True)
...
def payment_trip_handeler(trip_id,trans_id,amount):
    ...
    driver=trip.driver
    payment_trip=PaymentTrip.objects.create(trans_id=trans_id,amount=amount,trip=trip,confirmed=True)
