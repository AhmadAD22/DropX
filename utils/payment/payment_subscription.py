...
class PaymentDriverOrderSubscription:
    @classmethod
    def initiate_payment(self,subscription_req:DriverOrderSubscriptionPayment,request):
        url = 'https://api.edfapay.com/payment/initiate'
        subscription_number = str(subscription_req.pk)
        subscription_amount = str(subscription_req.price)
        subscription_currency = "SAR"
        merchant_password = str(os.getenv('MERCHANT_PASSWORD'))
        names = subscription_req.subscription.driver.fullName.split()
        firstName = names[0]
        lastName = names[-1]
        subscription_description=f'payment driver order subscription with %.2f {subscription_currency}' % subscription_req.price
        subscription_id=f"OS{subscription_req.id}"
        client_ip, _ = get_client_ip(request)
        if client_ip is None:
            # Unable to get the client's IP address
            return
        ...
        payload = {
            ...
            'payer_first_name': str(firstName),
            'payer_last_name': str(lastName),
            'payer_address': str(subscription_req.subscription.driver.address),
            'payer_country': 'SA',
            'payer_city': 'Riyadh',
            'payer_zip': '00000',
            'payer_email': str(subscription_req.subscription.driver.email),
            'payer_phone': str(subscription_req.subscription.driver.phone),
            'payer_ip': str(client_ip),
            'term_url_3ds': request.build_absolute_uri(reverse('finalize_payment')),
            'recurring_init': 'N',
            'auth': 'N',
...
def driver_order_payment_handeler(order_id,trans_id,amount):
    subscription_req=DriverOrderSubscriptionPayment.objects.get(pk=order_id)
    if subscription_req.subscription.paid==False:
        subscription=subscription_req.subscription
        subscription.paid=True
        subscription.save()
...
class PaymentDriverTripSubscription:
    @classmethod
    def initiate_payment(self,subscription_req:DriverTripSubscriptionPayment,request):
        url = 'https://api.edfapay.com/payment/initiate'
        subscription_number = str(subscription_req.pk)
        subscription_amount = str(subscription_req.price)
        subscription_currency = "SAR"
        ...
        firstName = names[0]
        lastName = names[-1]
        subscription_description=f'payment driver trip subscription with %.2f {subscription_currency}' % subscription_req.price
        subscription_id=f"TS{subscription_req.id}"
        client_ip, _ = get_client_ip(request)
        if client_ip is None:
            # Unable to get the client's IP address
            return
...
def driver_trip_payment_handeler(order_id,trans_id,amount):
    subscription_req=DriverTripSubscriptionPayment.objects.get(pk=order_id)
    if subscription_req.subscription.paid==False:
        subscription=subscription_req.subscription
        subscription.paid=True
        subscription.save()
